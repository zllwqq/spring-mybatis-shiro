...
import com.github.zllwqq.entity.FileInfo;
import com.github.zllwqq.service.WebUploader;

@Controller
@RequestMapping("/")
public class FileUploadController {
	
	private final Logger log = LogManager.getLogger(FileUploadController.class); 
	
	
	private WebUploader wu;
	
	public WebUploader getWu() {
		return wu;
	}

	@Autowired
	public void setWu(WebUploader wu) {
		this.wu = wu;
	}
	...
	public String fileUpload(String status, FileInfo info, @RequestParam(value = "file", required = false) MultipartFile file){
		...
		String uploadFolder = wu.getUploadFolder();
		...
		if(status == null){	//文件上传
			...
			//todo 待解决
			//报错：org.springframework.web.multipart.MultipartException: The current request is not a multipart request
			//进行check与merge操作时，request不包含文件，导致报错（spring 4.1.1版本未报错）
			if(status.equals("md5Check")){	//秒传验证，通过文件（二进制数据）md5签名判断是否已存在
	...
	@ResponseBody
	public String fileCheck(String status, FileInfo info){
		String uploadFolder = wu.getUploadFolder();
		String dateFolder = wu.getDateFolder();
		if(status.equals("md5Check")){	//秒传验证，通过文件（二进制数据）md5签名判断是否已存在
			...
			return "{\"status\": 1, \"path\": \"" + dateFolder+"/"+path + "\", \"message\": \"中文测试\"}";
		}
